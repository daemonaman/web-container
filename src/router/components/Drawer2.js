...
// import logo from "../../pages/image/hy.png"
import {
  all_admin_option,
  ...
  function sideBar(type, open) {
    ...
    if (type === "A") {
      ...
      return (
        <div style={{ marginTop: 1 }}>
          {side_top}
          {all_delar_user_options.map((option) => (
              <Link key={option.link} to={option.link} style={{ textDecoration: "none", }}   >
              <ListItem button key={option.name} className="drawer_text" >
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      {/* <AppBar position="fixed" open={true} style={{ backgroundColor: 'white' }}>
        <Toolbar style={{ justifyContent: "space-between" }}>
          ...
          {
            props.type === "A" ?
            <Typography variant="h6" noWrap component="div" style={{ color: '#08019f' }}>
              ADMIN PANEL
      ...
      <Drawer variant="permanent" open={true} >
        ...
        <Grid style={{ backgroundColor: '#eaf1f2', marginTop: -10,height:'110%' }}>
          <center>

         
            <Avatar src={{}} variant="rounded" alt="no_img" style={{ marginTop: "14px", marginLeft: 0, width: 95, height: 85 }}  />
            {/* <img  /> */}
